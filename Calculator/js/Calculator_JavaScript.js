...
function Input_Decimal(dot) {
    //This ensures that accidental clicking of the decimal pointer doesn't 
...
function Handle_Operator(Next_Operator) {
    ...
    //When an operator key pressed we convert the current number
    ...
    //Checl if an operator already exists and if Wait_Second_Operand is true,
    ...
    if (First_Operand == null) {
        ...
        //If operator exists, property lookop is performed for the operator
        //in the Perform_Calculation object and the function that matches the 
        //operator is exexcuted.
        let result = Perform_Calculation[operator](Value_Now, Value_of_Input);
        //Here we aff a fixed amount of numbers after the decimal.
...
keys.addEventListener('click', (event) => {
    //The target variable is an objecy that represents the element
    //that was clicked.
    const { target } = event;
